var fs    = require('fs'),
    path  = require('path'),
    utils = require('./utils'),
    taasClient = require('taas-client'),
...
function appendLocaleStrings(baseLocalePath, localeFileName) {
  ...
  if(taasEx) {
    contents += taasClient.i18n.toString();
    contents += taasClient.navigateHeirarchy.toString();
    contents += taasClient.lookupInLang.toString();
    contents += taasClient.makePseudo.toString();
  }
