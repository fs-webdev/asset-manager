...
proto.setContent = function setContent(newContent, type, skipMangle) {
  ...
  if(compressCSS && type === 'css') {
    ...
    this.contentRaw = this.content.toString('utf8');
    if(this.ext === 'styl') {
      console.log('stylus file before compress:',this.content.toString('utf8'));
      this.content = this.contentRaw;
    }
    else {
      try {
        // console.log('try compressCSSraw',compressCSSConfig, this.contentRaw);
        this.content = utils.compressCSS(this.content.toString('utf8'), compressCSSConfig);
        // console.log('try compressCSSmin',compressCSSConfig, this.content);
        console.log('Try compressCSS');
      } catch (e) {
        console.error("Error compressing file '" + this.meta.mainFile + ":" + e.message);
        this.content = this.contentRaw;
        console.log('error compressCSS',compressCSSConfig, this.content);
      }
    }
  }
};
