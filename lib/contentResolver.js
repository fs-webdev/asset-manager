...
proto.setCompressedCSSContent = function setCompressedCSSContent() {
  if(compressCSS){
    try {
      this.content = utils.compressCSS(this.content.toString('utf8'), compressCSSConfig);
    } catch (e) {
      console.error("Error compressing file '" + this.meta.mainFile + ":" + e.message);
    }
  }
 ...
 */
module.exports = function(paths, mPaths, doCompress, taasExOn) {
  searchPaths = paths || [];
  modulePaths = mPaths;
  taasEx = taasExOn;

  compressJS = doCompress || false;
  compressCSS = doCompress || false;
...
function readContentSync(meta, isSubAssembly) {
  ...
  if(meta.assembled) {
    try {
      ...
      if(baseLocalePathExists) {
        enFilePath = path.join(baseLocalePath, localeFileName + "_en.json");
        hasTranslations = fs.existsSync(enFilePath) || taasEx;
      }
...
function appendLocaleStrings(baseLocalePath, localeFileName) {
  ...
  contents += "var lang = $.extend({}, langs['en'], l1);\n\n";

  // at some point, the beginning of this function will be obselete, and we will only need the part below
  if(taasEx) {
    // contents += "var lang = {};\n\n";
    contents += taasTranslate.navigateHeirarchy.toString();
    contents += taasTranslate.injectData.toString();
    contents += taasTranslate.injectTaasTranslations.toString();
    contents += taasTranslate.lookupInLang.toString();
    contents += taasTranslate.makePseudo.toString();
  }
...
function appendTemplateHtml(baseModulePath, hasTranslations) {
  ...
  if(fs.existsSync(filePath)) {
    ...
    if (template) {
      if(taasEx) {
        template = utils.convertHTMLtoTaasJS(template, hasTranslations);
      } else {
