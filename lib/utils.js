var path     = require('path'),
    ...
    taasTranslate = require('taas-translate'),
 ...
 * Minify a CSS string.
 */
exports.compressCSS = function compressCSS(content, config){
  this.config = config || {};
  var minified = new CleanCSS(this.config).minify(content); // TODO: put in cleanCSS options
...
/**
 * Convert a string of html into a string that can be embeded in a javascript file with taas translations.
 */
exports.convertHTMLtoTaasJS = function convertHTMLtoTaasJS(html, hasLangResources){
  var sb = "";
  ...
  sb += taasTranslate.injectTaasTranslations.toString();

  sb += "\n\nfunction getSnippets(){\nvar snip = document.createElement('div');";
  if (hasLangResources) {
    sb += "\n$(snip).html(injectTaasTranslations(snippetsRaw));\n";
  } else {
    sb += "\n$(snip).html(snippetsRaw);\n";
  }
  sb += "\nreturn snip;\n}\n";
...
/**
 * Sort an array of paths so that they will be in a breadth first order (least number of
 * occurrences of the node_modules folder):
 * @param {string[]} paths - List of paths to sort.
 * @param {string} [order='asc'] - 'asc' for ascending or 'desc' for descending.
 *
 * @example
 * paths = ['reference/node_modules/theme-engage/node_modulesmyRepo', 'reference/node_modules/myRepo'];
 * breadthFirstPathSort(paths); // => ['reference/node_modules/myRepo', 'reference/node_modules/theme-engage/node_modulesmyRepo'];
 */
var nodeModulesRegex = /node_modules/g;
exports.breadthFirstPathSort = function(paths, order) {
  if (order === 'desc') {
     paths.sort(function(a, b) {
      return (b.match(nodeModulesRegex) || []).length - (a.match(nodeModulesRegex) || []).length
    });
  }
  else {
    paths.sort(function(a, b) {
      return (a.match(nodeModulesRegex) || []).length - (b.match(nodeModulesRegex) || []).length
    });
  }
}
