...
function start(config, cb) {
  ...
  try {
    manifest = JSON.parse(fs.readFileSync('builtAssets/manifest.json', 'utf-8'));
  } catch (err) {}

  // watch the manifest file for changes or deletion
  console.log('watching builtAssets/manifest.json for changes');
  fs.watchFile('builtAssets/manifest.json', function(curr, prev) {
