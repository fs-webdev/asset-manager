...
function start(config, cb) {
  ...
  if(config.inProd && fs.existsSync(manifestFile)) {
    ...
    context.resolvePath = function(route) {
...
function resolveInManifest(route){
  ...
  if(typeof route !== 'string') { //account for css routes coming in as objects that define different media types (e.g. print, screen, etc)
    for(var key in route) {
      if (asset.type === 'css') {
        if(key.toLowerCase() != 'screen') {
          path = path.replace("media='screen'", "media='" + key + "'");
        }
      }
      else if (asset.type === 'js') {
        var index = path.indexOf('></script>');
        path = path.substr(0, index) + asset.attribute + path.substr(index);
      }
    }
  }
  ...
  if(['css', 'js', 'img', 'html'].indexOf(pathParts[0]) !== -1) {
    ...
    if(asset){
      if(asset.type === 'css') {
        ...
        // express 3 uses res.sendfile while express 4 uses res.sendFile
        // need to use 'call' since saving a reference to the function loses the 'this'
        // scope of res and instead uses the 'this' scope of the current function
        (res.sendFile ? res.sendFile : res.sendfile).call(res, asset.getDiskPath());
      }
    } else {
