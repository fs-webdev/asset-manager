...
sproto.preprocessContent = function() {
  ...
  if (this.cr) {
    ...
    try {
    console.log('rendering stylus file: ', this.cr.getContent('utf-8'))
      var styl = stylus(this.cr.getContent('utf-8'))
        // .set('filename', this.file)
        .set('paths', stylusPaths);
      content = styl.render();
      console.log('finished rendering stylus file: ', content);
      this.cr.setContent(content, 'css', null, true);
      cproto.preprocessContent.call(this);
    //   console.log('styl.render',styl.render());
    //   console.log('this.contentx',this.cr.getContent('utf-8'));
    }
    catch (e) {
    ...
    //   console.log('finished rendering stylus file: ', this.cr.getContent('utf-8'));
    //   
    //   if (err && process && process.env && process.env.NODE_ENV !== 'production') {
    //     console.error("Error in Stylus File: " + self.cr.meta.mainFile);
    //     console.error(err);
    //   }
    //   else if(err) {
    //     console.log('post-stylus-render err', err);
    //   }
    //   else if(!err){
    //     // content = css;
    //     console.log('post-stylus-render content', css);
    //     console.log('CALLING CR.SETCONTENT');
    //     this.cr.setContent(css, 'css', null, true);
    //     cproto.preprocessContent.call(this);
    //   }
    // });
  }
