var fs              = require('fs'),
    ...
    // cleanCSSOptions = {
    //   benchmark: true //turns on benchmarking mode measuring time spent on cleaning up
    // },
...
cproto.writeContents = function writeContents(basePath, cb) {
  ...
  var finalPath = path.join(basePath, this.getRelativePath());
  minifyContents(content, function(err, minifiedContents) {
    utils.writeToFile(finalPath, minifiedContents, doGzip, cb);
  });
};

cproto.minifyContents = function minifyContents(content, cb) {
  var minified = new CleanCSS(cleanCSSOptions).minify(content, function(errors, minified){
    if(errors) {
      cb(content); // if errors, return original content
    }
    else {
      console.log('minified', minified);
      cb(minified); // if no errors, return minified content
    }
  });
};
