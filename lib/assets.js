...
// cproto.getClientManifestEntry = function getClientManifestEntry() {
//   var entry = aproto.getClientManifestEntry.call(this);
//   entry.name = this.name;
//   return entry;
// };
...
cproto.writeContents = function writeContents(basePath, cb) {
  ...
  var that = this;
  aproto.writeContents.call(that, basePath, function writeCompressedCB(){
    var finalPath = path.join(basePath, that.getRelativeRawPath());
    utils.writeToFile(finalPath, that.cr.getContentRaw('utf8'), doGzip, cb);
    console.log('writing raw CSS file:', finalPath);
    // console.log('writing raw CSS content:', that.cr.getContentRaw('utf8'));
  });
};
